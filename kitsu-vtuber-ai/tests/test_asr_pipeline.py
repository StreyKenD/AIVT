...
@pytest.mark.asyncio
async def test_pipeline_emits_partial_and_final_events() -> None:
    config = ASRConfig(
        model_name="tiny.en",
    ...
    vad = PatternVAD([True] * 8 + [False] * (config.silence_threshold_frames + 2), config.frame_bytes)
    ...
    await pipeline.process(iter_frames(frames, delay=config.frame_duration_ms / 1000))
